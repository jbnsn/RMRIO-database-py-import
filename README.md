...
Extract the *.zip-files into the folder `rmrio-data` and run `main.py`. The class `rmrio_import` can be loaded in `main.py` with the parameters `rmrio_file_path` and `rmrio_year` specified e.g.
